<?php
 ...
 class Tjnets extends Permissions
 {	
	/**
	 * 会员列表首页  
	 */  
	 ...
	 */
	 ...
	 { 
		...
		if ($dtp1<=$dtp){
            ...
            $rs111=Db::name('member')->where('refree',$tj_username)->select();
			$i=1;
			$c_num=count($rs111);
            ...
            foreach($rs111 as $rs)
			{
				...
				if ($c_num>1)
				{
				 ...
				 $str=$str.$rankname.'</td>';
				 if($rs['status']==0) {
					 $str=$str.' <tr><td align="center" ><span style=" color:red">未激活</span></td></tr>';
					 }
				$str=$str.'<tr><td align="center" >'.date("Y-m-d",$rs["register_time"]).'</td></tr>';
				$str=$str.'</table></td></tr></table>';
			
				$rs1=Db::name('member')->where('refree',$rs["user_number"])->select();
						...
						if ($tjrs==1){
							$str=$str.'
								<table align="center" border="0" cellspacing="0" cellpadding="0">
								  <tr>
									<td align="center"><img src="/static/admin/images/line.gif" alt="" width="8" border="0" style="WIDTH: 1px; HEIGHT: 20px" /></td>
								  </tr>
								</table>
								';
							}
